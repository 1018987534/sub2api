...
func TestFirstTokenPriorityOrderWithStats(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "crossing thirteen seconds creates a separate fast pool",
			ids:  []int64{1, 2},
			stats: map[int64]FirstTokenLatencyStats{
				1: stats(13_100, 5, time.Minute),
				2: stats(12_900, 5, time.Minute),
			},
			expected: []int64{2, 1},
		},
		{
...
func TestFirstTokenPriorityDefaultStickyEligible(t *testing.T) {
	now := time.Now()
	reliable := FirstTokenLatencyStats{PredictedMS: 13_000, SampleCount: 3, UpdatedAt: now}
	require.True(t, firstTokenPriorityDefaultStickyEligible(reliable, now))

	reliable.PredictedMS = 13_001
