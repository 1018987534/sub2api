...
func controlPlaneEnabled(cfg *config.Config) bool {
	return cfg == nil || cfg.IsControl()
}
...
// ProvideImageTaskService 构造一次性图片任务服务。任务元数据可短暂写入
// Redis 供浏览器轮询，生成结果只保存在当前进程内并在首次轮询时消费，
// 不写入 Redis 或对象存储。
func ProvideImageTaskService(store ImageTaskStore, settings *ImageStorageSettingService) *ImageTaskService {
	return NewImageTaskServiceBrowserMemory(store, defaultImageTaskTTL, defaultImageTaskExecutionTimeout)
}
...
func ProvideChannelMonitorV2Aggregator(repo ChannelMonitorV2Repository, db *sql.DB, settingService *SettingService, cfg *config.Config) *ChannelMonitorV2Aggregator {
	...
	if !controlPlaneEnabled(cfg) || os.Getenv("CHANNEL_MONITOR_V2_DISABLE_AGGREGATOR") == "1" {
		return aggregator
	}
	aggregator.Start()
	return aggregator
