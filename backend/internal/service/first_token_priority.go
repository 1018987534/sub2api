...
const (
	...
	firstTokenPriorityExplorePercent = uint64(5)
...
func (s *RateLimitService) ObserveFirstTokenLatency(ctx context.Context, account *Account, requestID string, firstTokenMs *int) {
	if s == nil || s.firstTokenLatencyStatsCache == nil || account == nil || firstTokenMs == nil || account.ID <= 0 || *firstTokenMs <= 0 {
		return
	}
...
func firstTokenPriorityExplore(seed string) bool {
	hasher := fnv.New64a()
	_, _ = hasher.Write([]byte(seed))
	return hasher.Sum64()%100 < firstTokenPriorityExplorePercent
}

func firstTokenPrioritySeed(ctx context.Context, accountIDs []int64) string {
	seed, _ := ctx.Value(ctxkey.RequestID).(string)
	if strings.TrimSpace(seed) == "" {
		seed, _ = ctx.Value(ctxkey.ClientRequestID).(string)
	}
	if strings.TrimSpace(seed) == "" {
...
// ordered by predicted TTFT for exploitation; an exploration request promotes
// the stalest unknown candidate so recovered and new accounts keep being sampled.
func firstTokenPriorityOrder(ctx context.Context, candidates []*Account, cache FirstTokenLatencyStatsCache) []int64 {
	...
	return firstTokenPriorityOrderWithStats(
		accountIDs,
		stats,
		time.Now(),
		firstTokenPriorityExplore(firstTokenPrioritySeed(ctx, accountIDs)),
	)
}
