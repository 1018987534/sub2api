...
func TestLockAndMergeAccountExtraPreservesLatestCodexTicket(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT.*FOR NO KEY UPDATE`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "rate_conversion_ratio", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "opencode_group_unchanged", "opencode_auto", "opencode_snapshot", "current_extra"}).
			AddRow(true, false, true, nil, nil, nil, nil, nil, nil, nil, false, nil, nil, []byte(`{"codex_turn_ticket:model":{"state":"latest-database-ticket"},"old_admin_setting":true}`)))
	...
	extra, _, err := lockAndMergeAccountProbeExtra(context.Background(), client, account, nil, nil, nil)
	require.NoError(t, err)
...
func TestLockAndMergeAccountExtraDegradesOnUnparsableExtra(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT.*FOR NO KEY UPDATE`).
		WithArgs(int64(41), service.PlatformOpenAI, service.AccountTypeOAuth, `{"access_token":"test"}`, nil).
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "rate_conversion_ratio", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "opencode_group_unchanged", "opencode_auto", "opencode_snapshot", "current_extra"}).
			AddRow(true, false, true, nil, nil, nil, nil, nil, nil, nil, false, nil, nil, []byte(`[1,2,3]`)))
