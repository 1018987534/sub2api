...
func (r *usageLogRepository) GetUsageTrendWithUsageFilters(ctx context.Context, startTime, endTime time.Time, granularity string, filters UsageLogFilters) (results []TrendDataPoint, err error) {
	return r.getUsageTrendWithFilters(ctx, startTime, endTime, granularity, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.InstanceID, filters.UpstreamModelMismatch, filters.NativeCompactionV2)
}

func (r *usageLogRepository) getUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, instanceID string, upstreamModelMismatch *bool, nativeCompactionV2 *bool) (results []TrendDataPoint, err error) {
	if shouldUsePreaggregatedTrend(granularity, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, billingMode, instanceID, upstreamModelMismatch, nativeCompactionV2) {
	...
	query, args = appendUsageLogInstanceQueryFilter(query, args, instanceID, "")
...
func shouldUsePreaggregatedTrend(granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string, optional ...any) bool {
	var instanceID string
	var upstreamModelMismatch, nativeCompactionV2 *bool
	// Accept both the pre-instance-id call shape used by older tests and the
	// current (instance_id, upstream_mismatch, native_compaction) shape.
	if len(optional) == 2 {
		if value, ok := optional[0].(string); ok {
			instanceID = value
			upstreamModelMismatch, _ = optional[1].(*bool)
		} else {
			upstreamModelMismatch, _ = optional[0].(*bool)
			nativeCompactionV2, _ = optional[1].(*bool)
		}
	} else if len(optional) >= 3 {
		instanceID, _ = optional[0].(string)
		upstreamModelMismatch, _ = optional[1].(*bool)
		nativeCompactionV2, _ = optional[2].(*bool)
	}
...
func (r *usageLogRepository) GetModelStatsWithUsageFiltersBySource(ctx context.Context, startTime, endTime time.Time, filters UsageLogFilters, source string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, source, filters.BillingMode, filters.InstanceID, filters.UpstreamModelMismatch, filters.NativeCompactionV2)
}

func (r *usageLogRepository) getModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, source string, billingMode string, instanceID string, upstreamModelMismatch *bool, nativeCompactionV2 *bool) (results []ModelStat, err error) {
...
func (r *usageLogRepository) GetGroupStatsWithUsageFilters(ctx context.Context, startTime, endTime time.Time, filters UsageLogFilters) (results []usagestats.GroupStat, err error) {
	return r.getGroupStatsWithFilters(ctx, startTime, endTime, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.InstanceID, filters.UpstreamModelMismatch, filters.NativeCompactionV2)
}

func (r *usageLogRepository) getGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string, instanceID string, upstreamModelMismatch *bool, nativeCompactionV2 *bool) (results []usagestats.GroupStat, err error) {
	query := `
		SELECT
	...
	query, args = appendUsageLogInstanceQueryFilter(query, args, instanceID, "ul")
	if upstreamModelMismatch != nil {
...
func (r *usageLogRepository) GetUserBreakdownStats(ctx context.Context, startTime, endTime time.Time, dim usagestats.UserBreakdownDimension, limit int) (results []usagestats.UserBreakdownItem, err error) {
	...
	query, args = appendUsageLogInstanceQueryFilter(query, args, dim.InstanceID, "ul")
	if dim.UserID > 0 {
		query += fmt.Sprintf(" AND ul.user_id = $%d", len(args)+1)
