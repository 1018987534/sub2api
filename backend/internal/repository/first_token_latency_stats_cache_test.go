...
func TestTotalLatencyStatsCacheUsesThirteenSecondFastPoolBoundary(t *testing.T) {
	_, _, cache := newTotalLatencyTestCache(t)
	ctx := context.Background()

	recordTotalLatencySamples(t, cache, 16, "slow-boundary", repeatedDurations(22, 16_000))
	stats, err := cache.GetStatsBatch(ctx, []int64{16})
	require.NoError(t, err)
	require.Equal(t, 16_000.0, stats[16].PredictedMS)
	require.False(t, stats[16].ReliableFast, "16-second accounts must stay out of the special-price fast pool")

	recordTotalLatencySamples(t, cache, 13, "fast-boundary", repeatedDurations(22, 13_000))
	stats, err = cache.GetStatsBatch(ctx, []int64{13})
	require.NoError(t, err)
	require.Equal(t, 13_000.0, stats[13].PredictedMS)
	require.True(t, stats[13].ReliableFast, "13 seconds is the inclusive fast-pool boundary")
}
