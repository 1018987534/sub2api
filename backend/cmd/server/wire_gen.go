...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	tempUnschedFailureCounterCache := repository.NewTempUnschedFailureCounterCache(redisClient)
	firstTokenLatencyStatsCache := repository.NewFirstTokenLatencyStatsCache(redisClient)
	timeoutCounterCache := repository.NewTimeoutCounterCache(redisClient)
	openAI403CounterCache := repository.NewOpenAI403CounterCache(redisClient)
	geminiTokenCache := repository.NewGeminiTokenCache(redisClient)
	compositeTokenCacheInvalidator := service.NewCompositeTokenCacheInvalidator(geminiTokenCache)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, tempUnschedFailureCounterCache, firstTokenLatencyStatsCache, timeoutCounterCache, openAI403CounterCache, settingService, compositeTokenCacheInvalidator)
	...
	channelService := service.ProvideChannelService(channelRepository, groupRepository, apiKeyAuthCacheInvalidator, pricingService, accountRepository, usageLogRepository)
	...
	channelMonitorUserHandler := handler.NewChannelMonitorUserHandler(channelMonitorService, channelService, settingService)
