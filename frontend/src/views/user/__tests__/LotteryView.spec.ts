...
const { getAnnouncement, getCurrent, getRounds, getPublicRounds, join, showError, showSuccess } = vi.hoisted(() => ({
  getAnnouncement: vi.fn(),
  getCurrent: vi.fn(),
  getRounds: vi.fn(),
  getPublicRounds: vi.fn(),
  join: vi.fn(),
  showError: vi.fn(),
  showSuccess: vi.fn(),
}))
...
vi.mock('@/api/lottery', () => ({
  default: { getAnnouncement, getCurrent, getRounds, getPublicRounds, join },
}))
...
function winner(id: number, roundNo = 2) {
  return { id, round_id: roundNo, round_no: roundNo, email: `winner-${id}@example.com`, prize_amount: 5, awarded_at: '2026-08-29T13:00:00Z', participated_at: '2026-08-29T12:00:00Z' }
}
...
describe('LotteryView snapshot', () => {
  beforeEach(() => {
    ...
    getCurrent.mockResolvedValue({ enabled: true, current_round: round, joined: false, eligibility: { eligible: true, total_recharge: 0 }, recent_winners: recentWinners, my_recent_winners: [] })
    getRounds.mockResolvedValue({ items: [], total: 0, pages: 0 })
    getPublicRounds.mockResolvedValue({ items: [], total: 0, pages: 0 })
  })

  it('loads the public endpoint and frames only the previous round without changing row spacing', async () => {
    const wrapper = mount(LotteryView, { props: { snapshot: true }, global: { stubs: { AppLayout: { template: '<div><slot /></div>' }, Icon: true, 'router-link': true } } })
    await flushPromises()
    ...
    expect(wrapper.text()).toContain('winner-6@example.com')
    ...
    expect(wrapper.find('button[aria-label="common.nextPage"]').exists()).toBe(false)
  })
  ...
  it('never draws the previous-round frame on the actual lottery page', async () => {
    const wrapper = mount(LotteryView, { global: { stubs: { AppLayout: { template: '<div><slot /></div>' }, Icon: true, 'router-link': true } } })
    await flushPromises()
