        ...
        <section v-if="selected && draft" class="space-y-5">
          <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-dark-700 dark:bg-dark-900">
            ...
            <div class="grid gap-4 md:grid-cols-2">
              ...
              <label class="field"><span>专用 API Key ID</span><input v-model.number="draft.api_key_id" type="number" min="0" placeholder="必填：当前管理员拥有的有效 Key"></label>
          ...
          <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-dark-700 dark:bg-dark-900">
            <div class="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 class="font-semibold text-gray-900 dark:text-white">最近 7 天记录</h2><p class="text-xs text-gray-500">答案仅管理员点击详情时可见。</p></div><div class="flex gap-2"><button class="btn-secondary" :disabled="running || saving || dirty || !draft.version || !draft.api_key_id" @click="run">{{ running ? '已排队…' : '立即检测' }}</button><button class="btn-secondary" :disabled="historyLoading" @click="loadHistory()">刷新</button></div></div>
...
import { getIntelligenceConfigs, getIntelligenceHistory, runIntelligenceCheck, saveIntelligenceConfig, type IntelligenceConfig, type IntelligenceRecord } from '@/api/intelligence'
...
async function run() {
  if (!selected.value || dirty.value || running.value || !draft.value?.version) return
...
onUnmounted(() => {
  disposed = true; historyGeneration++
