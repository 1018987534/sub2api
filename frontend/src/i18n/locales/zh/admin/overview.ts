export default {
    ...
    dashboard: {
      ...
      firstTokenLatencyDescription: '只统计成功计费的真实流式请求。正常总耗时取选定窗口内最新样本的算术平均值；主窗口达到“最少样本”后使用主窗口，否则回退到保留的 24 小时样本。快池按倍率优先，慢池按总耗时由短到长调度。',
      firstTokenAlgorithmDescription: '判定规则：样本耗时 <= 快阈值计为快，>= 慢阈值计为慢，中间区间只参与平均值不计入两类占比。单条完成耗时 > 熔断秒数时，已在快池的账号立即移入慢池；单条等于熔断值不触发。快池退出还需同时满足：最近 3 分钟窗口跨度至少 2 分钟、慢请求占比至少 70%、窗口总耗时中位数达到慢阈值、且相对窗口外基线至少 1.5 倍并至少多 5 秒；这里不再要求最近 3 分钟至少 10 条样本。慢池恢复需同时满足：最近 3 分钟跨度至少 1 分钟、至少 4 条样本、快请求占比至少 75%、窗口总耗时中位数不高于快阈值；恢复期间进入预备状态并按 30 秒探测。未达到最少样本时不确认快池。普通探测间隔以 2 分钟为基准，按相对最快账号的耗时平方和连续变慢次数递增，单次最长 1 小时；熔断或预备状态固定 30 秒探测。若一个分组全部是慢池，核心调度仍按总耗时排序，只对其余账号按每个 2 分钟探测槽位公平累积探测债务，周期随账号数增长并封顶 1 小时。',
      firstTokenCacheRateDescription: '缓存率统计近 24 小时缓存读取 token / 输入 token（输入 + 缓存创建 + 缓存读取），无有效样本显示“-”。',
      firstTokenFastThreshold: '快阈值',
      firstTokenSlowThreshold: '慢阈值',
      firstTokenSampleLimit: '样本上限',
      firstTokenMinimumSamples: '最少样本',
      firstTokenPrimaryWindow: '主窗口',
      firstTokenSingleSampleCircuit: '单条熔断',
      firstTokenThresholdInvalid: '阈值必须是 1-3600 秒的整数，且慢阈值必须大于快阈值',
      firstTokenThresholdSaved: '耗时阈值已保存，最多 5 秒内在各节点生效',
      firstTokenThresholdSaveFailed: '耗时阈值保存失败',
      firstTokenLatencyFailed: '总耗时统计加载失败',
      firstTokenLatencyEmpty: '当前启用的上游中转账号暂无总耗时统计',
      firstTokenAccount: '账号',
      firstTokenPrediction: '正常总耗时',
      firstTokenPercentiles: 'P50 / P90',
      firstTokenPool: '调度池',
      firstTokenFastPool: '快池',
      firstTokenSlowPool: '慢池',
      firstTokenSlowPoolRecovering: '慢池 · 预备恢复 {current}/{total}',
    ...
    lottery: {
      ...
      rules: '轮次规则',
      participantThreshold: '达到多少人开奖',
      prizeCount: '中奖人数 / 礼品数量',
      prizeAmount: '每人奖励金额',
      drawMode: '开奖方式',
      automatic: '自动开奖',
      manual: '手动开奖',
      ...
      minRecharge: '累计充值金额',
      minRechargeHint: '填写 0 表示不限制。',
      minAccountAge: '注册满天数',
      recentRecharge: '隐藏规则：近几天充值过',
