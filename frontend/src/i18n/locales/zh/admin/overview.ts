export default {
    ...
    users: {
      ...
      reengagement: {
        action: '召回邮件（{count}）',
        actionAll: '召回筛选结果（{count}）',
        title: '发送老用户召回邮件',
        allActivity: '全部使用状态',
        allRechargeStatuses: '全部充值状态',
        recharged: '已充值',
        notRecharged: '未充值',
        sendScope: '发送范围',
        selectedScope: '已选用户（{count}）',
        allScope: '筛选结果全部（{count}）',
        activityLabel: '召回范围',
        inactiveDays: '近 {days} 天未使用',
        neverUsed: '从未使用',
        recipients: '已选择用户',
        template: '邮件模板',
        templateName: '老用户召回',
        ...
        queueAll: '后台发送全部',
        sending: '发送中...',
        confirm: '确定向所选的 {count} 个用户发送召回邮件吗？系统会再次校验使用时间、角色和账号状态。',
        confirmAll: '确定向当前筛选结果中的 {count} 个用户发起召回邮件吗？系统会在后台重新筛选有效普通用户并分批发送。',
        backgroundHint: '全部发送将在后台分批执行，不受当前分页限制；当天已发送或已退订的用户会自动跳过。',
        queuedSuccess: '已提交后台发送，共匹配 {count} 个用户',
        success: '召回邮件处理完成：发送 {sent} 封，跳过 {skipped} 个用户',
        partialFailure: '其中 {count} 封发送失败，请检查 SMTP 配置和服务日志。',
        failed: '发送召回邮件失败'
      },
      editUser: '编辑用户',
      deleteUser: '删除用户',
