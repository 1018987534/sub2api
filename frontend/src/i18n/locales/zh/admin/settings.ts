export default {
    settings: {
      ...
      defaults: {
        ...
        defaultSignupAPIKeyGroup: '注册默认 API Key 分组',
        defaultSignupAPIKeyGroupHint: '新用户注册时自动创建的 API Key 将加入此分组；只影响之后注册的新用户。',
        defaultSignupAPIKeyGroupAuto: '自动选择活动的 GPT 分组（兼容旧配置）',
      ...
      site: {
        ...
        hideUserSubscriptionsMenu: '隐藏用户订阅菜单',
        hideUserSubscriptionsMenuHint: '启用后普通用户侧边栏将隐藏“我的订阅”入口。'
      },
      purchase: {
        title: '充值/订阅页面',
      ...
      gatewayRouting: {
        ...
        healthProtection: '节点离线自动保护',
        healthProtectionHint: '单节点监控样本过期且仍有其他健康节点时，自动将该节点生效比例降为 0；恢复上报后自动回到目标比例',
        packetLossProtection: '连续丢包自动保护',
        packetLossProtectionHint: '多数探测目标连续两轮失败，或连续两个短窗口丢包达到 40% 时，暂停该节点的新请求。只在有健康替代节点时触发，现有请求继续执行。',
        packetLossCooldown: '丢包暂停时长（分钟）',
        packetLossCooldownHint: '1–120 分钟，默认 5 分钟；修改应用于下次暂停。到期且连续三轮探测成功后恢复，持续严重丢包会再次暂停。',
        packetLossCooldownError: '丢包暂停时长必须为 1–120 的整数',
        packetLossUntil: '暂停截止 {time}',
        packetLossStates: {
          unknown: '丢包数据不足或过期',
          observing: '丢包观察中',
          healthy: '近期探测正常',
          paused: '连续丢包，暂停分流',
          awaiting_recovery: '暂停已到期，等待网络恢复确认',
          no_healthy_peer: '检测到丢包，暂无健康替代节点',
          state_unavailable: '丢包保护状态暂不可用'
        },
        threshold: '自动摘流阈值',
        monitorUrl: '监控数据地址',
        node: '节点',
        targetWeight: '目标比例',
        effectiveWeight: '生效比例',
    maxConcurrency: '并发上限',
    currentConcurrency: '当前并发',
    overflowNode: '满载兜底节点',
    overflowRandom: '按剩余健康节点随机',
    overflowNodeHint: '首选节点达到并发上限时优先改投此节点；该节点也满载后再随机选择其他健康节点。',
    requests: '个请求',
    unlimitedConcurrency: '不限',
    capacityUnavailable: '暂不可用',
        traffic: '套餐流量',
        status: '状态',
        targetTotal: '目标比例合计',
        targetTotalError: '目标比例必须合计为 100%（当前为 {total}%）',
        weightHint: '请填写各节点百分比，目标比例合计必须等于 100%；流量保护摘流只影响生效比例。',
        monitorStale: '部分监控数据不可用或已过期；可用节点会继续分流，异常节点可能已被自动摘流。',
        unlimited: '不限流量',
        dataStale: '数据过期',
        statuses: {
          active: '分流中',
          ...
          auto_disabled_monitor_stale: '监控过期已摘流',
          auto_disabled_monitor_missing: '监控缺失已摘流',
          auto_disabled_monitor_record_unavailable: '监控记录不可用已摘流',
          auto_disabled_packet_loss: '连续丢包已暂停',
