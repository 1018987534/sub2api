export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '此列显示本地实际采用的上游计费倍率；配置折算比例时，原始声明会先折算再参与利润控制、低倍率调度、排序和账号成本计算。Sub2API 无法验证上游声明是否与实际扣费一致，请结合账单、余额变化和实际用量自行核验。',
        autoProbe: '自动探测上游声明倍率',
        autoProbeHint: '启用后按全局周期刷新上游声明倍率；此开关本身不会修改账号倍率。',
        syncRate: '同步上游声明倍率',
        syncRateHint: '成功探测后按“上游声明基准倍率 × 折算比例”生成本地上游倍率，并统一用于账号倍率、利润控制和低倍率调度；探测失败或结果超出允许范围时保持不变。开启本项会同时打开“自动探测上游声明倍率”。',
        syncRateManagedHint: '当前账号倍率与本地上游倍率按“上游声明基准倍率（不含高峰）× 折算比例”统一维护。',
        rateConversionRatio: '上游倍率折算比例',
        rateConversionRatioHint: '本地上游倍率 = 上游声明基准倍率 × 折算比例，并用于账号成本、利润控制和低倍率调度。例如上游 2x、折算比例 0.05，最终按 0.1x 计算。',
        rateConversionRatioInvalid: '上游倍率折算比例必须大于 0 且不超过 100',
        syncedRateTooltip: '该账号倍率与调度使用的上游倍率由原始声明折算后统一同步',
        manualProbe: '立即探测上游倍率',
        stale: '已过期',
        unsupported: '不支持',
        failed: '失败',
        notProbed: '未探测',
        conversionSummary: '折算：上游声明 {declared}x × {ratio} = 本地基准 {converted}x',
      ...
      tempUnschedulable: {
        ...
        hint: '配置上游调用失败时，账号临时停止参与调度的规则。',
        notice: '规则按顺序匹配，需同时满足错误码与关键词。',
        modeRules: '按错误规则',
        modeConsecutiveFailures: '按连续失败',
        modeFirstTokenLatency: '按首字延迟占比',
        failureNotice: '每次调用失败会同时累计所有规则；成功一次会清零。若同时命中多条，按最长暂停时间执行。',
        addRule: '添加规则',
        addFailureRule: '添加连续失败规则',
        ruleOrder: '规则序号',
        ...
        failureRulesInvalid: '请至少填写一条有效的连续失败规则，窗口、次数和暂停时间均为必填正整数。',
        ...
        triggerMode: '触发方式',
        consecutiveFailureSummary: '{window} 秒内连续失败 {count}/{threshold} 次',
        firstTokenThreshold: '本次首字耗时',
        firstTokenThresholdSummary: '{observed} 秒（阈值 {threshold} 秒）',
        firstTokenRatioTrigger: '{minutes} 分钟内慢请求 {slow}/{total}，实际占比 {observed}%，达到触发占比 {threshold}%。',
        errorMessage: '错误详情',
        reset: '恢复状态',
