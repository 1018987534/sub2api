export default {
  ...
  keys: {
    ...
    groupLabel: '分组',
    selectGroup: '选择分组',
    groupRoutesLabel: '分组路由',
    groupRoutesHint: '按首选、备用的顺序依次尝试可用分组。',
    groupRoutesEmpty: '请选择首选分组后，再配置有序备用分组。',
    primaryGroup: '首选',
    backupGroup: '备用 {index}',
    backupShort: '备用',
    addBackupGroup: '添加备用',
    removeBackupGroup: '移除备用分组',
    moveBackupUp: '上移备用分组',
    moveBackupDown: '下移备用分组',
    maxMultiplier: '倍率上限',
    maxAcceptedMultiplier: '可接受的最高倍率',
    unlimitedMultiplier: '不限制',
    maxMultiplierHint: '填写此密钥可接受的最高实际倍率（例如 0.20x）。留空表示不设上限；分组当前实际倍率高于该值时，会跳过并按顺序尝试下一个备用分组。',
    invalidGroupRoutes: '每个分组路由都必须设置有效分组，倍率上限必须大于 0。',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
    noKeysYet: '暂无 API 密钥',
    ...
    manageGroupRoutes: '管理有序分组路由',
    manageGroupRoutesTitle: '管理分组路由',
    manageGroupRoutesFor: '仅调整“{name}”的首选分组、倍率上限和备用顺序。',
    groupRoutesUpdatedSuccess: '分组路由已更新',
    manage: '管理',
