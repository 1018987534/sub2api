export default {
    ...
    dashboard: {
      ...
      firstTokenLatencyDescription: 'Uses completed, billable streams only. Normal total duration is the arithmetic average of the newest samples in the selected window; the primary window is used once it reaches the minimum sample count, otherwise the retained 24-hour data is used. The fast pool is ordered by multiplier; the slow pool is ordered by shorter duration.',
      firstTokenAlgorithmDescription: 'Decision rules: samples at or below the fast threshold count as fast, samples at or above the slow threshold count as slow, and the middle band contributes only to the average and neither ratio. A completed duration strictly greater than the single-sample circuit value immediately moves a fast-pool account to the slow pool; equality does not trip it. Fast-pool exit requires all of: a recent 3-minute window spanning at least 2 minutes, at least 70% slow requests, a recent median at or above the slow threshold, and a recent median at least 1.5x the pre-window baseline plus 5 seconds. The recent window no longer requires 10 samples. Slow-pool recovery requires all of: a recent 3-minute window spanning at least 1 minute, at least 4 samples, at least 75% fast requests, and a recent median at or below the fast threshold; the account enters a recovery-candidate state and is probed every 30 seconds. Fast-pool confirmation is withheld below the minimum sample count. Normal probe cadence starts at 2 minutes, grows with the squared duration ratio to the fastest account and consecutive slow observations, and is capped at 1 hour; circuit-broken or recovery-candidate accounts are probed every 30 seconds. When a group is entirely slow-pool, core scheduling still follows duration order; only probes are shared fairly with a 2-minute slot per remaining account, growing with group size and capped at 1 hour.',
      ...
      firstTokenFastThreshold: 'Fast threshold',
      firstTokenSlowThreshold: 'Slow threshold',
      firstTokenSampleLimit: 'Sample limit',
      firstTokenMinimumSamples: 'Minimum samples',
      firstTokenPrimaryWindow: 'Primary window',
      firstTokenSingleSampleCircuit: 'Single-sample circuit',
      firstTokenThresholdInvalid: 'Thresholds must be integer seconds from 1 to 3600, and slow must be greater than fast',
      firstTokenThresholdSaved: 'Duration thresholds saved; all nodes apply them within 5 seconds',
      firstTokenThresholdSaveFailed: 'Failed to save duration thresholds',
      ...
      firstTokenPrediction: 'Normal total',
      firstTokenPercentiles: 'P50 / P90',
      firstTokenPool: 'Pool',
      firstTokenFastPool: 'Fast',
      firstTokenSlowPool: 'Slow',
      firstTokenSlowPoolRecovering: 'Slow · recovery candidate {current}/{total}',
    ...
    lottery: {
      ...
      rules: 'Round rules',
      participantThreshold: 'Participants to trigger draw',
      prizeCount: 'Winners / prize quantity',
      prizeAmount: 'Reward per winner',
      drawMode: 'Draw mode',
      ...
      minRechargeHint: 'Use 0 for no minimum.',
      minAccountAge: 'Minimum account age in days',
      recentRecharge: 'Hidden rule: recharged within recent days',
