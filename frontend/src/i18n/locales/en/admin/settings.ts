export default {
    settings: {
      ...
      defaults: {
        ...
        defaultSignupAPIKeyGroup: 'Default Signup API Key Group',
        defaultSignupAPIKeyGroupHint: 'API keys created automatically during signup are assigned to this group. This only affects future signups.',
        defaultSignupAPIKeyGroupAuto: 'Auto-select the active GPT group (legacy behavior)',
      ...
      site: {
        ...
        hideUserSubscriptionsMenu: 'Hide user subscriptions menu',
        hideUserSubscriptionsMenuHint: 'When enabled, the "My Subscriptions" entry is hidden from regular users.'
      },
      purchase: {
      ...
      gatewayRouting: {
        ...
        healthProtection: 'Automatic node health protection',
        healthProtectionHint: 'When one node has a stale monitoring sample and another healthy node is available, set only that node’s effective share to zero; restore its target share after fresh samples return',
        packetLossProtection: 'Sustained packet loss protection',
        packetLossProtectionHint: 'Pause new requests when a majority of probe targets fail twice in a row or show at least 40% loss in two consecutive short windows. A healthy alternative is required; existing requests continue.',
        packetLossCooldown: 'Packet loss pause (minutes)',
        packetLossCooldownHint: '1–120 minutes, default 5. Changes apply to the next pause. Resume after the deadline and three successful probe rounds; sustained severe loss renews the pause.',
        packetLossCooldownError: 'Packet loss pause must be an integer from 1 to 120',
        packetLossUntil: 'Paused until {time}',
        packetLossStates: {
          unknown: 'Packet loss data insufficient or stale',
          observing: 'Observing packet loss',
          healthy: 'Recent probes healthy',
          paused: 'Routing paused due to packet loss',
          awaiting_recovery: 'Pause elapsed; awaiting confirmed recovery',
          no_healthy_peer: 'Packet loss detected; no healthy alternative',
          state_unavailable: 'Packet loss protection state unavailable'
        },
        threshold: 'Automatic removal threshold',
        monitorUrl: 'Monitoring data URL',
        node: 'Node',
        targetWeight: 'Target share',
        effectiveWeight: 'Effective share',
    maxConcurrency: 'Concurrency limit',
    currentConcurrency: 'Current concurrency',
    overflowNode: 'Capacity fallback node',
    overflowRandom: 'Random healthy node',
    overflowNodeHint: 'When the selected node reaches its limit, try this node first, then choose another healthy node at random if it is also full.',
    requests: 'requests',
    unlimitedConcurrency: 'Unlimited',
    capacityUnavailable: 'Unavailable',
        traffic: 'Plan traffic',
        status: 'Status',
        targetTotal: 'Target share total',
        targetTotalError: 'Target shares must total 100% (current: {total}%).',
        weightHint: 'Enter each node as a percentage. Target shares must total 100%; traffic protection only changes the effective share.',
        monitorStale: 'Some monitoring data is unavailable or stale. Healthy nodes keep routing, and unhealthy nodes may have been automatically removed.',
        unlimited: 'Unlimited',
        ...
        statuses: {
          ...
          auto_disabled_monitor_stale: 'Removed · monitoring stale',
          auto_disabled_monitor_missing: 'Removed · monitoring missing',
          auto_disabled_monitor_record_unavailable: 'Removed · monitoring records unavailable',
          auto_disabled_packet_loss: 'Paused · sustained packet loss',
