...
// OpenAI
const openaiModels = [
  ...supplementalOpenAIModels.map(model => model.id),
  // GPT-5.2 系列
...
const openaiPresetMappings = [
  ...supplementalOpenAIModels.map(model => ({ label: model.name, from: model.id, to: model.id, color: 'bg-sky-100 text-sky-700 hover:bg-sky-200 dark:bg-sky-900/30 dark:text-sky-400' })),
